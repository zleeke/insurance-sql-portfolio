...
SELECT
  ...
  ROUND(CASE WHEN SUM(p.premium)=0 THEN NULL ELSE SUM(pm.payment_amount)/SUM(p.premium) END,4) AS loss_ratio
FROM policies p
JOIN customers c ON p.customer_id = c.customer_id
LEFT JOIN claims cl ON cl.policy_id = p.policy_id
LEFT JOIN payments pm ON pm.claim_id = cl.claim_id
GROUP BY p.policy_type, c.state
