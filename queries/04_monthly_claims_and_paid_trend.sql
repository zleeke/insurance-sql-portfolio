...
WITH claim_months AS (
  SELECT strftime('%Y-%m', date(claim_date)) AS month,
...
payment_months AS (
  SELECT strftime('%Y-%m', date(payment_date)) AS month,
...
FROM months m
LEFT JOIN claim_months cm ON m.month = cm.month
LEFT JOIN payment_months pm ON m.month = pm.month
ORDER BY m.month;
